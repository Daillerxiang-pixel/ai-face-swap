// =============================================
// AI换图 管理后台 - PC端公共组件库
// 请先加载 config.js（定义 getAdminApiBase / ADMIN_API_BASE）
// =============================================

// config.js 已定义 getAdminApiBase()；此处覆盖为统一实现，读取 config.js 设置的 ADMIN_API_BASE
...
const API = {
  ...
  async request(method, url, data = null) {
    ...
    const res = await fetch(getAdminApiBase() + url, opts);
    if (res.status === 401) {
  ...
  async postForm(url, formData) {
    const res = await fetch(getAdminApiBase() + url, {
      method: 'POST',
