...
- **只维护一套后端业务代码**（本仓库）；测试与正式 **不是**两套并行实现。
- **流向**：本地开发完成 → **先**上 **测试服** → 验证通过 → **再**将 **同一 Git 提交** 上 **正式服**。常态下测试服版本 **先于或等于** 正式服（新功能先测后发）。
- **两边仅在三处区分**：**部署环境**（机器、目录、端口、PM2、`server/.env` 等）、**域名**（DNS / HTTPS）、**数据**（各自独立库）。除此以外不另搞「测试专用业务分支代码」。

### 发布流程原则（须遵守）

1. **单一代码库**：**不存在**「测试服后端一套代码、正式服另一套代码」的并行仓库；仓库只有一个，测试机与正式机部署 **同一 Git 提交** 的产物。与上一节一致：差别限于 **环境、域名、数据**。
2. **两套数据库、两套数据**：测试与正式 **各自独立** 的数据库文件（见 §2 / §3 路径），**不是**同一份库复制给两边随便互拷。正式库为 **用户生产数据**，**禁止**随意清空、用测试库覆盖、或无备份的破坏性操作；测试库可重置、可造数。Schema 随迁移与代码对齐；**数据内容**不要求两边一致。
3. **先在测试环境验证**：变更（代码、依赖、数据库迁移、配置）须先在 **测试服务器**（§3，`test1.kanashortplay.com`）部署，并完成约定测试（接口/页面、客户端连测试服、关键业务流程等）。
4. **测试通过后再上正式**：确认无阻塞后，将 **已在测试服验证的同一套提交** 部署到 **正式服务器**（§2）——即从测试验证结果**晋级**到正式，而非维护两套分支。
5. **禁止**：未在测试环境验证通过即直接修改或发布正式机；不因排期单独跳过测试环节。
...
| **Gitea（私有化 Git）** | **仅本测试机**：`https://test1.kanashortplay.com:8443`，仓库 `…/aihuantu/ai-face-swap`。**正式服不跑 Gitea**，全项目只保留这一份实例。 |
